...
//
//
// * It is not original version of iRate.m. It have altered by Manikandan ( manikandan.m@npcompete.com )
...
//        self.applicationVersion = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
        self.applicationVersion = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"];
        NSLog(@"Budle version %@",self.applicationVersion);
...
//    }
    ...
    else if ( self.usesCount < self.usesUntilPrompt)
    {
        if (self.verboseLogging)
        {
            NSLog(@"iRate did not prompt for rating because the app has only been used %i times ", (int)self.usesCount);
        }
        return NO;
    }
...
//    }
    ...
    else if (self.lastReminded != nil && [[NSDate date] timeIntervalSinceDate:self.lastReminded] < self.remindPeriod * SECONDS_IN_A_DAY)
    {
        if (self.verboseLogging)
        {
            NSLog(@"Time interval %f and period %f",[[NSDate date] timeIntervalSinceDate:self.lastReminded],self.remindPeriod * SECONDS_IN_A_DAY);
...
- (void)applicationLaunched
{
    ...
    if (self.verboseLogging)
    {
        NSLog(@"This Version login, Uses count %d",self.usesCount);
    }
...
- (void)openRatingsPageInAppStore
{
    if (_displayAppUsingStorekitIfAvailable && [SKStoreProductViewController class])
    {
        ...
        {
            if (self.mainRootViewController && [self.mainRootViewController isKindOfClass:[UIViewController class]])
                rootViewController = self.mainRootViewController;
        }
